...
export class MepDatepicker {
  ...
  selectDate(date) {
    if (!date.isDisabled) {
      ...
      if (this.withTime) {
        if (this.selectedDates.length && this.isRange) {
          selectedDate.add(2, 'hour')
          this.toHour.value = selectedDate.format('hh')
          this.toMin.value = selectedDate.format('mm')
          this.toAMPM.value = selectedDate.format('A')
        } else {
          selectedDate.add(1, 'hour')
          this.fromHour.value = selectedDate.format('hh')
          this.fromMin.value = selectedDate.format('mm')
          this.fromAMPM.value = selectedDate.format('A')
        }
      }
  ...
  render() {
    return (
      <div>
        <div onClick={this.showHide.bind(this)} class="mep-date-content">
        ...
        <div class={this.showPicker ? "mep-datepicker show" : "mep-datepicker"} >
          <div>
          ...
          {
            this.isRange && this.withTime &&
            <div class="time">
              <select ref={el => this.fromHour = el}>
                {
                  this.timeHours.map(t => <option value={t}>{t}</option>)
                }
              </select>
              <select ref={el => this.fromMin = el}>
                {
                  this.timeMinutes.map(t => <option value={t}>{t}</option>)
                }
              </select>
              <select ref={el => this.fromAMPM = el}>
          ...
          {this.isRange &&
            this.withTime &&
            <div class="time">
              <select ref={el => this.toHour = el}>
                {
                  this.timeHours.map(t => <option value={t}>{t}</option>)
                }
              </select>
              <select ref={el => this.toMin = el}>
                {
                  this.timeMinutes.map(t => <option value={t}>{t}</option>)
                }
              </select>
              <select ref={el => this.toAMPM = el}>
                <option value="AM">AM</option>
                <option value="PM">PM</option>
              </select>
            </div>
